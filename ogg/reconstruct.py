        ...
        for i, filename in enumerate(bin_files, 1):
            ...
            for page_bytes, bos, pageno in parse_ogg_pages_from_buffer(buf):
                ...
                if last_page_no is None:
                    if not bos:
                        raise RuntimeError(
                            f"Flux invalide: première page non-BOS (pageno={pageno})"
                        )
                else:
                    if pageno != last_page_no + 1:
                        raise RuntimeError(
                            f"Discontinuité: pageno={pageno}, "
                            f"attendu={last_page_no + 1} (sur {filename})"
                        )
            ...
            if i <= 3:
                print(f"[chunk {i}] {filename}: bytes={len(data)} pages={chunk_pages}")
            elif i % 50 == 0:
                print(f"[progress] {i}/{len(bin_files)} pages_out={pages_out}")

    out_size = os.path.getsize(output_ogg)
    print(f"[done] bytes_in={bytes_in:,} pages_out={pages_out:,} out_size={out_size:,}")

    if pages_out == 0:
        os.remove(output_ogg)
        raise RuntimeError("Aucune page Ogg extraite")


def reconstruct_ogg_from_file(input_file: str, output_ogg: str):
    data = open(input_file, "rb").read()
    if not data:
        raise RuntimeError("Fichier d'entrée vide")

    if data[:4] != b"OggS":
        raise RuntimeError(
            f"Codec non reconnu: le fichier ne commence pas par OggS "
            f"(first16={data[:16].hex(' ')})"
        )

    data, skipped = skip_spotify_custom_page_if_present(data)
    if skipped:
        print("[info] Custom Spotify Ogg page skipped")
    ...
    print(f"[done] pages_out={pages_out:,} out={output_ogg}")


def main():
    ap = argparse.ArgumentParser()
    g = ap.add_mutually_exclusive_group(required=True)
    g.add_argument("--input-dir", help="Répertoire contenant decrypt_*.bin")
    g.add_argument("--input-file", help="Fichier Ogg déjà concaténé")
    ap.add_argument("--output", required=True)
    args = ap.parse_args()

    if args.input_file:
        reconstruct_ogg_from_file(args.input_file, args.output)
    else:
        reconstruct_ogg_from_chunks(args.input_dir, args.output)
